          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
            <div>
              ...
              <h2 className="heading-1 mb-6">
                ...
                <em className="italic text-accent">We Build Legacies</em>
              </h2>
              <span className="divider-accent block mb-6" />
            ...
            <div className="relative hidden lg:block">
              ...
              />
              <div className="absolute -bottom-6 -left-6 bg-accent rounded-2xl px-8 py-6"
        ...
        <div className="container-site">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {VALUES.map(({ icon: Icon, title, description }) => (
              <div key={title} className="bg-white rounded-2xl p-8 border border-neutral-200"
                style={{ boxShadow: 'var(--shadow-card)' }}>
                <div className="w-14 h-14 rounded-xl bg-cream border border-neutral-200 flex items-center justify-center mb-6">
      ...
      <section className="section">
        <div className="container-site">
          ...
          <div className="max-w-2xl mx-auto relative">
            {/* Spine line */}
            <div className="absolute top-0 bottom-0 w-0.5 bg-neutral-200"
              style={{ left: 'calc(6rem + 1px)' }} />
            <div className="flex flex-col gap-10">
              {MILESTONES.map(({ year, event, desc }) => (
                <div key={year} className="flex gap-8 items-start">
                  <div className="w-24 shrink-0 text-right font-display text-xl font-bold text-accent pt-0.5">
                    {year}
                  </div>
                  <div className="relative pl-8">
                    {/* Dot */}
                    <div className="absolute -left-1.5 top-1.5 w-3.5 h-3.5 rounded-full bg-accent"
      ...
      <section className="section-cream" id="team">
        <div className="container-site">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {AGENTS.map(agent => (
              <div key={agent._id} className="bg-white rounded-2xl overflow-hidden border border-neutral-200"
                style={{ boxShadow: 'var(--shadow-card)' }}>
                ...
                <div className="p-6">
                  ...
                  {agent.bio && <p className="text-sm text-neutral-500 leading-relaxed mb-5">{agent.bio}</p>}
                  <div className="flex gap-6">
                    {agent.listings && (
                      <div>
                        <div className="font-display text-xl font-bold text-primary">{agent.listings}</div>
                        <div className="text-xs text-neutral-400 uppercase tracking-wider">Active</div>
                      </div>
                    )}
                    {agent.soldProperties && (
                      <div>
                        <div className="font-display text-xl font-bold text-primary">{agent.soldProperties}</div>
                        <div className="text-xs text-neutral-400 uppercase tracking-wider">Sold</div>
                      </div>
                    )}
                  </div>
                </div>
